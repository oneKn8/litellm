...
def _claim(api_key_id: str, job_id: str = "job-9", stopped_at=None) -> MagicMock:
    """A key row a prior job holds, still claiming the one-active-per-key index unless stopped."""
    return _spec_record({"api_key_id": api_key_id, "job_id": job_id, "stopped_at": stopped_at})
...
def _shadow_prisma(claimed=(), agg_rows=None, by_key_rows=None, known_keys=("key-hash", "key-hash-2")) -> MagicMock:
    ...
    async def find_claimed_keys(where: dict, **_: object):
        ...
        stopped rows a real partial index would have released."""
        scoped = [row for row in claimed if row.api_key_id in where["api_key_id"]["in"]]
        if "stopped_at" not in where:
            return scoped
        return [row for row in scoped if row.stopped_at is where["stopped_at"]]
...
async def test_start_shadow_eval_creates_job_and_frees_expired_or_exhausted_ones(monkeypatch: pytest.MonkeyPatch):
    ...
    release the one-active-per-key index so a new eval can start. Every requested key is
    swept and claimed on its own row, and each gets its own copy of the turn budget."""
    import litellm.proxy.proxy_server as proxy_server
    ...
    assert [call.args[1] for call in prisma.db.execute_raw.call_args_list] == ["key-hash", "key-hash-2"]
