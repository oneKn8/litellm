...
Anthropic Messages, and Responses API surfaces, each normalized to chat shape), duplicates each
...
def _responses_final_text(response_obj: object) -> str:
    ...
    if any(
        ...
        return ""
    return str(getattr(response_obj, "output_text", "") or "")
...
class _SurfaceOps:
    ...
    unknown call types fail closed."""

    __slots__ = ("chat_request", "final_text")
...
_CHAT_OPS: Final = _SurfaceOps(_chat_request_from_chat, _chat_final_text)
_ANTHROPIC_OPS: Final = _SurfaceOps(_chat_request_from_anthropic_messages, _chat_final_text)
_RESPONSES_OPS: Final = _SurfaceOps(_chat_request_from_responses, _responses_final_text)
...
class ShadowEvalLogger(CustomLogger):
    ...
    async def _run_shadow_eval(
        ...
        try:
            ...
            shadow: Final = await self._call_router_shadow(job.router_name, messages, shadow_params, parent_metadata)
